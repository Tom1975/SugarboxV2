...
Z80Desassember::Z80Desassember(EmulatorEngine* machine) : machine_(machine)
{
   m_NbFlags = 0;
...
Z80Desassember::~Z80Desassember()
{
}



void Z80Desassember::ToggleFlag(unsigned short addr)
{
   for (int i = 0; i < m_NbFlags; i++)
   {
      if (m_Flags[i] == addr)
      {
         // remove it
         memcpy(&m_Flags[i], &m_Flags[i + 1], (m_NbFlags - i - 1) * sizeof(short));
         m_NbFlags--;
         if (m_IndexFlag >= m_NbFlags && m_NbFlags != 0) m_IndexFlag = m_NbFlags - 1;
         return;
      }

      if (m_Flags[i] > addr)
      {
         if (m_NbFlags == NB_MAX_FLAGS) return;   // No more flag available
         // insert it here
         memcpy(&m_Flags[i + 1], &m_Flags[i], (m_NbFlags - i) * sizeof(short));
         m_Flags[i] = addr;
         m_NbFlags++;
         return;
      }
   ...
   if (m_NbFlags == NB_MAX_FLAGS) return;   // No more flag available
   m_Flags[m_NbFlags++] = addr;
}

bool Z80Desassember::IsFlagOnAdress(unsigned short addr)
{
   for (int i = 0; i < m_NbFlags; i++)
   {
      if (m_Flags[i] == addr) return true;
      if (m_Flags[i] > addr) return false;
   }
   return false;
}

void Z80Desassember::ToggleBreakpoint(unsigned short addr)
{
   if (breakpoint_handler_)
   {
      breakpoint_handler_->ToggleBreakpoint(addr);
   }
}

bool Z80Desassember::IsBreakOnAdress(unsigned short addr)
{
   if (breakpoint_handler_)
   {
      return (breakpoint_handler_->IsThereBreakOnAdress(addr));
   }
   return false;
}

void Z80Desassember::RemoveAllFlags()
{
   if (m_NbFlags > 0)
   {
      m_NbFlags = 0;
...
unsigned short Z80Desassember::GetMaxedPreviousValidAdress(unsigned short Addr_P)
{
...
unsigned short Z80Desassember::GetPreviousValidAdress(unsigned short Addr_P)
{
   Z80* z80 = machine_->GetProc();
   unsigned short address = Addr_P;
...
void Z80Desassember::InitOpcodeShortcuts()
{
